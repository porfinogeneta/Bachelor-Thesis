...
class Game:
    ...
    def is_colliding_with_apple(self, snake: Snake, direction: tuple[int, int]) -> bool:
        is_colliding = False
    ...
    def run(self):
        ...
        while running:
            ...
            ate_apple = self.is_colliding_with_apple(snake=self.snake, direction=direction)
            if (ate_apple):
                [print(pos) for pos in self.snake.body]
            ...
            # update snake position only when it's moving
            if (direction != (0, 0)):
