...
from src.consts import TRAIN_VAL_SPLIT, GAMES_IN_RAW_FILE
...
# split_point = int(len(all_encoded_ids) * TRAIN_VAL_SPLIT)
# choose split point on the first game in the last 10% of data

all_ids_np = np.array(all_encoded_ids)
# choose indexes of start tokens
start_idxs = np.where(all_ids_np == 0)[0]

split_point = int(len(start_idxs) * TRAIN_VAL_SPLIT)
assert len(start_idxs) == GAMES_IN_RAW_FILE, "Start tokens should be as much as games"
