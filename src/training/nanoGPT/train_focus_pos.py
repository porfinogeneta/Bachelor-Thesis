...
# MODIFICATION
def create_target_mask(targets):
    """
    Create a mask for targets where only tokens in the specified range are kept.
    Tokens outside the range are set to -100 (ignored by CrossEntropyLoss).
    
    Args:
        targets: tensor of shape (batch_size, seq_len) containing target token IDs
    
    Returns:
        masked_targets: tensor with same shape, but out-of-range tokens set to -100
    """
    # Create a mask for tokens in the target range
    in_range_mask = (targets >= target_token_min) & (targets <= target_token_max)
    
    # Create masked targets - keep target tokens for in-range tokens, set others to -100
    masked_targets = targets.clone()
    masked_targets[~in_range_mask] = -100  # -100 is ignored by CrossEntropyLoss
...
def get_batch(split):
    ...
    # MODIFICATION
    y = create_target_mask(y)  # Apply the target mask to y
...
# MODIFICATION
def compute_loss_and_metrics(logits, targets):
    """
    Compute loss and additional metrics for token range prediction.
    ...
    Returns:
        loss: cross entropy loss (only for non-ignored tokens)
    ...
    # Compute loss (automatically ignores -100 tokens)
...
def estimate_loss():
    ...
    for split in ['train', 'val']:
        losses = torch.zeros(eval_iters)
        total_metrics = {'valid_targets': 0, 'total_targets': 0, 'correct': 0}
        
        for k in range(eval_iters):
            X, Y = get_batch(split)
            with ctx:
                logits, _ = model(X, Y)  # We'll compute loss ourselves
                loss, metrics = compute_loss_and_metrics(logits, Y)
...
while True:
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        with ctx:
            ...
            logits, _ = model(X, Y)  # We'll compute loss ourselves
            # Compute custom loss with masking
            loss, _ = compute_loss_and_metrics(logits, Y)
            loss = loss / gradient_accumulation_steps # scale the loss to account for gradient accumulation
        # immediately async prefetch next batch while model is doing the forward pass on the GPU
        X, Y = get_batch('train')
        # backward pass, with gradient scaling if training in fp16
